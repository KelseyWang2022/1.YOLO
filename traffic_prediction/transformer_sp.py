...
class ConvTransformer(nn.Module):
    ...
    def forward(self, src):
        ...
        src = src.permute(1, 0, 2)  # Change to (seq_len, batch_size, hidden_dim)
        output = self.transformer_encoder(src)
        output = output.permute(1, 0, 2)  # Change back to (batch_size, seq_len, hidden_dim)
...
# Train the model
num_epochs = 100  # Increasing training epochs to better observe convergence
train_losses = []
test_losses = []
...
# Ensure we have collected losses before plotting
if len(train_losses) > 0 and len(test_losses) > 0:
    # Plotting the training and test loss
    train_losses = [x * 100 for x in train_losses]  # Multiplying by 100 to scale the loss values for better visualization
    test_losses = [x * 100 for x in test_losses]
...
if len(train_losses) > 0 and len(test_losses) > 0:
    # Plotting the training and test loss
    #train_losses和test_losses各自乘1000，以便更好地可视化
    train_losses = [x * 100 for x in train_losses]
    test_losses = [x * 100 for x in test_losses]
...
# ... (rest of the code for metric calculation)